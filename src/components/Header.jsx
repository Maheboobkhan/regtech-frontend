...
class Navbar extends Component {
  ...
  getUserByToken = async (token) => {
    ...
    try {
        ...
        // Optionally check if the user exists in the response
        if (response.data !== '' && response.data.access_token === token) {
            this.setState({isTokenCorrect: true});
        }
        else{
          this.setState({isTokenCorrect: false});
        }
    } catch (error) {
  ...
  render() {
    ...
    const { isMenuOpen, isNavbarFixed, isOpen, isServicesDropdownOpen, token, isTokenCorrect } = this.state;
      ...
      <nav className={`md:px-10 px-2 py-2 fixed top-0 left-0 w-full transition-all duration-300 ${isNavbarFixed ? 'bg-[#f1c2d9] border-gray-300 z-50' : 'bg-[#fff3f9] z-10'}`}>
        <div className="container mx-auto flex justify-between items-center">
          ...
          <ul className={`hidden md:flex items-center space-x-4 mb-2 ${isNavbarFixed ? 'text-black' : 'text-gray-400'}`}>
            ...
            {this.state.isTokenCorrect ? <button className='ml-10 hidden md:flex items-center px-4 py-1.5 font-bold hover:border-none hover:underline hover:text-[#e27daa] bg-white font-myfont text-[#e27daa] border-2 border-[#e27daa] cursor-pointer transition-all duration-400'>
