...
const FaceMatchRegtech = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${domain}/facematch`, formData, { headers });
      // console.log(res)
      setResponse(res);
    } catch (err) {
            ...
            <>
              {(response?.data?.status_code === 200 || response?.status === 200) && (
                <div className="bg-green-100 text-green-800 p-3 rounded mt-4">
                  ...
                  <p><strong>Matched: </strong>{(response?.data.rec_face?.result && response?.data.rec_face?.result === true) ? "TRUE" : "FALSE"}</p>
                </div>
              )}
