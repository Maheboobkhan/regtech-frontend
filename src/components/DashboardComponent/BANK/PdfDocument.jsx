...
const renderTable = (header, data, columns) => (
  <View style={[styles.panel, styles.breakPage]}>
    ...
    <View style={styles.table}>
      <View style={styles.tableRow}>
        {columns.map((col, index) => {
          // Check if the column is "description" and apply specific width
          const isDescriptionColumn = col === "description";
          return (
            <Text
              key={index}
              style={[
                styles.tableCell,
                styles.tableHeader,
                isDescriptionColumn ? { flex: 4 } : { flex: 1 }, // Set flex for description column
              ]}
            >
              {col}
            </Text>
...
const StatementPDF = ({
  ...
  <Document>
    <Page size={[1000, 1080]} style={styles.page}>
      {" "}
      {/* Adjust size here */}
      {/* Header Section */}
      <View style={styles.headerContainer}>
