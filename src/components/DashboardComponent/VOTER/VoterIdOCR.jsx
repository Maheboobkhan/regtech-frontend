...
//         'https://regtechapi.in/api/voter_ocr',
//         formData,
//         {
//           headers: {
//             'AccessToken': token,
...
const VoterUpload = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post(`${domain}/voter_ocr`, formData, {
        headers: {
          AccessToken: token,
          "Content-Type": "multipart/form-data",
        },
      ...
      setError(
        err.response ? err.response.data.message : "Error uploading file"
      );
    } finally {
      setLoading(false);
    }
          ...
          {response && response.name && (
            <div className="mt-6">
              <div className="bg-green-100 text-green-800 p-4 rounded">
                <h3 className="text-2xl font-semibold">Voter Details</h3>
                <div className="mt-4 flex flex-col gap-y-2">
                  <p><strong className="text-lg">Voter Description:&nbsp;&nbsp;</strong>
                    {response.raw_ocr_texts && response.raw_ocr_texts.length > 0
                      ? response.raw_ocr_texts.join(' ')
                      : 'null'}
                  </p>
                  {/* <p>
                    <strong>Voter Description:&nbsp;&nbsp;</strong>
                    <span
                      dangerouslySetInnerHTML={{
                        __html:
                          response.raw_ocr_texts &&
                          response.raw_ocr_texts.length > 0
                            ? response.raw_ocr_texts.join("<br />") // Join with <br /> for line breaks
                            : "null",
                      }}
                    />
                  </p> */}
                  <p>
                    <strong>Name:&nbsp;&nbsp;</strong>
                    {response.name ? response.name : "null"}
                  </p>
                  <p>
