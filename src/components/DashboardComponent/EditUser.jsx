...
class UserEdit extends Component {
  ...
  fetchApiOptionsWithId = async () => {
    ...
    try {
      ...
      data.forEach((option) => {
        ...
        console.log("custom_plan: ", custom_plan);

        this.setState({ planAmount: plan_amount });
        //   console.log(scheme_price, payment_slab)

        // // Initialize the object to store the results

        // // Split the scheme price and payment slab by commas
        if (custom_plan === "yes") {
          ...
          console.log("selectedPlanaa: ", this.state.selectedPlanTypes);
        }
  ...
  fetchSchemeTypes = async () => {
    ...
    try {
      ...
      const data = await response.json();
      console.log("fetchSchemeTypes: ", data);
  ...
  fetchApiGroups = async (token) => {
    ...
    try {
      ...
      const data = await response.json();
      console.log("fetchApiGroups: ", data);
  ...
  fetchApiOptions = async (token) => {
    ...
    try {
      ...
      console.log("apiOptions: ", response);
      const data = await response.json();
  ...
  handleAddCustomInput = (groupId, optionId) => {
    ...
    console.log("addCustom: ", this.state.customPlanInputs);
  };
  ...
  handleRemoveCustomInput = (optionId, index) => {
    ...
    console.log("RemovecustomPlanInputs: ", this.state.customPlanInputs);
  };
  ...
  addCustomInput = (optionId) => {
    ...
    console.log("AddcustomPlanInputs: ", this.state.customPlanInputs);
  };
  ...
  handlePlanTypeChange = (optionId, event) => {
    ...
    // console.log('selectedPlanType: ',this.state.selectedPlanType);
  };
                    ...
                    >
                      ...
                      <div className="flex space-x-4">
                        ...
                        {/* {this.state.selectedPlanTypes[option.id] ===
                          "Custom plan" && (
                          <div>
                            {customPlanInputs[option.id]?.map(
                              (input, index) => (
...
//         plan: data.plan || "",
//         getPlan: data.plan || "",
//         planDuration: this.state.plans[data.plan].planDuration,
//       });
...
//       // Process the fetched data
//       data.forEach((option) => {
//         const { id, api_group_id, scheme_price, custom_plan, plan_amount, payment_slab } = option;
...
//         newEnabledInputs[id] = true;
//         newInputValues[id] = scheme_price || ""; // Assuming scheme_price is the value to be displayed
//         Custom[id] = custom_plan === "yes" ? "Custom plan" : custom_plan === "no" ? "API plan":"Nothing Selected"
//         console.log('cutsttttt: ',Custom)
//         CustomPlan[id] = scheme_price;
...
//       // Update the state with the processed data
//       this.setState({
//         customPlanInputs: CustomPlan,
//         selectedPlanTypes: Custom,
//         checkedOptions: newCheckedOptions,
//         enabledInputs: newEnabledInputs,
//         inputValues: newInputValues,
//         inputData: newInputData,
...
//       console.log('selectedPlanTypes: ',this.state.selectedPlanTypes);
//       console.log('customplaninputs: ',this.state.customPlanInputs);
