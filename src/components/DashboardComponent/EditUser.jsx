...
class UserEdit extends Component {
    ...
    try {
      ...
      data.forEach((option) => {
        ...
        if (custom_plan === "yes") {
          ...
          // console.log('schemePrices: ',schemePrices);
          // const paymentSlabs = option.payment_slab.split(",").map(Number); // Convert to numbers
          // console.log('paymentSlabs: ',paymentSlabs);

          const schemePrices = option.scheme_price ? option.scheme_price.split(",").map(Number) : [];

          const paymentSlabs = option.payment_slab ? option.payment_slab.split(",").map(Number) : [];
          ...
          this.setState((prevState) => {
            ...
            // console.log('uniqueNewInputs: ',uniqueNewInputs)
  ...
  handleApiOptionChange = (optionId, groupId, isChecked) => {
    ...
    if (!isChecked) {
      // Delete the specific option data from inputData using optionId and groupId
      delete this.state.inputData[`${optionId}_${groupId}`];
      
      // Reset the selectedPlanTypes for this optionId
      this.state.selectedPlanTypes[optionId] = "Nothing Selected";
    
      // Remove the data from customPlanInputs if the optionId is matched
      this.setState((prevState) => {
        const updatedCustomPlanInputs = { ...prevState.customPlanInputs };
    
        // Delete the key for the optionId if it exists
        delete updatedCustomPlanInputs[optionId];
    
        return {
          customPlanInputs: updatedCustomPlanInputs,
        };
      });
    }
