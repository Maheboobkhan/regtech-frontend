  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        'https://regtechapi.in/api/electricity',
        {
          id_number: idNumber,
        },
        {
        ...
        <div className="p-4">
          ...
          {response && response.statusCode === "200" && response.electricity.response.isValid === "Yes" && (
            <div className="bg-green-100 text-green-800 p-3 rounded mt-4">
              ...
              <p><strong>Customer ID:</strong> {response.electricity.response.customer_id}</p>
              <p><strong>Customer Name:</strong> {response.electricity.response.customer_name}</p>
              <p><strong>Operator:</strong> {response.electricity.response.operator_name} - {response.electricity.response.operator_code}</p>
              <p><strong>Due Amount:</strong> {response.electricity.response.due_amount}</p>
              <p><strong>Due Date:</strong> {response.electricity.response.due_date}</p>
            </div>
          )}
        </div>
      </div>
