...
//         `http://localhost:8000/api/getallusers/${token}`
//       );
...
//     //         user.scheme_type.toLowerCase().includes(searchQuery)
//     //     );
//     // }
...
//                                         <td className='border-b'><button className={`px-2 py-1 cursor-text ${user.verified === 0 ? 'text-white bg-red-600 rounded-md md:text-base text-sm' : 'pr-9 rounded-md bg-green-500'} `}>{user.verified === 0 ? 'Not Verified': 'Verified'}</button></td>
//                                         <td className='border-b'><button className={`px-2 py-1 ${user.status === 0 ? 'text-white bg-red-600 rounded-md hover:bg-red-500' : 'pr-5 rounded-md hover:bg-green-400 bg-green-500'}`}>{user.status === 0? 'inactive': 'active'}</button></td>
...
//                                             <span className="cursor-pointer text-xl hover:text-red-400 text-red-500 mx-1" onClick={this.deleteUser(user.id)}><MdDelete /></span>
//                                             <span className="cursor-pointer text-xl hover:text-gray-500 text-gray-700 mx-1"><MdOutlineLockReset /></span>
//                                         </td>
//                                     </tr>
//                                 ))}
...
class UserList extends Component {
    ...
    confirmDelete = async () => {
        ...
        try {
            const response = await axios.post(`http://localhost:8000/api/userdelete/${userToDelete}`, {
            });
