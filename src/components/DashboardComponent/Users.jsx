...
class UserList extends Component {
    ...
    ActiveInactiveUser = async (id) => {
        const domain = localStorage.getItem('domain');

        try {
            const response = await axios.post(`${domain}/useractiveinactive/${id}`, {
            });
            ...
            this.setState({ error: "Error update user" });
        }
    };
                            ...
                            <tbody>
                                {paginatedUsers.map((user, index) => (
                                    <tr key={user.id}>
                                        ...
                                        <td className='border-b'>
                                            <button className={`px-2 py-1 ${user.status === 0 ? 'text-white bg-red-600 rounded-md hover:bg-red-500' : 'pr-5 rounded-md hover:bg-green-400 bg-green-500'}`} onClick={()=>this.ActiveInactiveUser(user.id)}>
