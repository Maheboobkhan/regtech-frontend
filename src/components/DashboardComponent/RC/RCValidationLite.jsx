...
//       const res = await axios.post('https://regtechapi.in/api/rc_validation', body, { headers });
//       console.log(res);
...
//           <h3 className="text-xl font-semibold text-white">RC Validation Lite</h3>
//           <Link
...
const RcValidationLite = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // const res = await axios.post('https://regtechapi.in/api/rc_validationlite', body, { headers });
      const res = await axios.post('https://regtechapi.in/api/rc_validationlite', body, { headers });
      // console.log(res);
      setResponse(res.data);
    } catch (err) {
    ...
    <div className="flex justify-center items-center min-h-screen bg-gradient-to-r from-green-50 via-blue-50 to-purple-50">
      <div className="w-full max-w-lg bg-white shadow-lg rounded-lg overflow-hidden">
        ...
        <div className="p-4">
          ...
          {response && (
            <>
              {response[0]?.rc_validation?.status_code === 200 && (
                <div className="bg-green-100 text-green-800 p-3 rounded mt-4">
                  <h3 className="text-xl font-bold mb-4">RC Details</h3>
                  <hr />
                  <p><strong>Owner Name:</strong> {response[0].rc_validation.data.owner_name}</p>
                  <p><strong>RC Number:</strong> {response[0].rc_validation.data.rc_number}</p>
                  <p><strong>Model Name:</strong> {response[0].rc_validation.data.model_name}</p>
                  <p><strong>Manufacturing Date:</strong> {response[0].rc_validation.data.manufacturing_date}</p>
                  <p><strong>Chassis Number:</strong> {response[0].rc_validation.data.vehicle_chasi_number || 'N/A'}</p>
                </div>
              )}
              {response?.statusCode === 102 && (
                <div className="bg-red-500 text-white p-3 rounded mt-4">
