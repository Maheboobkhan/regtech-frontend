...
class SchemeTypeEdit extends Component {
  state = {
    id: null,
    name: '',
    hitLimit: '',
  ...
  handleSubmit = async (event) => {
    ...
    const { id, name, hitLimit } = this.state;
    ...
    try {
      ...
      toast.error('An error occurred while updating the scheme');
      this.setState({ submitting: false });
    }
  };
    ...
    return (
      <div className="p-8 bg-gray-100 min-h-screen flex items-center justify-center">
        <div className="bg-white p-6 rounded-lg shadow-lg w-full max-w-md">
          <h2 className="text-2xl font-bold text-black mb-4">Edit Scheme</h2>
          <form onSubmit={this.handleSubmit}>
            ...
            <button
              ...
              disabled={submitting}
            >
              {submitting ? 'Submitting...' : 'Update Scheme'}
            </button>
          </form>
        </div>
        <ToastContainer />
...
export default withRouter(SchemeTypeEdit);
