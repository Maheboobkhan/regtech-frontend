  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await axios.post(
        'https://regtechapi.in/api/ckyc_searchadvance',
        {
          pano: panNumber,
          client_ref_num: 7856,  // Replace with actual client reference number if available
          dob: dob,
          identifier_type: 'PAN'
        },
        {
    ...
    <div className="flex justify-center items-center min-h-screen bg-gradient-to-r from-green-50 via-blue-50 to-purple-50">
      <div className="w-full max-w-lg bg-white shadow-lg rounded-lg overflow-hidden">
        <div className="bg-[#00acc1] p-4 flex justify-between items-center">
          <h3 className="text-xl font-semibold text-white">Ckyc Search Advanced</h3>
          <Link
            to="/dashboard/kyc/ckysearch_advance_api"
            ...
            <>
              {response.statusCode === '200' && (
                <div className="bg-green-400 text-white p-3 rounded mt-4">
                  ...
                  {/* Adjust the response data fields according to the API response structure */}
              ...
              {(response.statusCode === '404' || response.statusCode === 400) && (
                <div className="bg-red-500 text-white p-3 rounded mt-4">
                  {response.message || 'Server Error, Please try later.'}
                </div>
              )}
