...
const CommunityArea = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post('https://regtechapi.in/api/community_area', formData, { headers });
      // console.log(res)
    ...
    <div className="flex justify-center items-center min-h-screen bg-gradient-to-r from-gray-100 to-gray-300">
      <div className="w-full max-w-lg bg-white shadow-lg rounded-lg overflow-hidden">
        <div className="bg-blue-500 p-4 flex justify-between items-center">
          ...
          <Link
            to="/dashboard/kyc/community_area_api"
        ...
        <div className="p-4">
          ...
          {response && response.statusCode === 200 && (
            <div className="bg-green-100 text-green-800 p-3 rounded mt-4">
              <h3 className="text-2xl font-semibold">Community Area Details</h3>
              <div className="mt-4 flex flex-col gap-y-2">
                <p><strong>Title:</strong> {response.data.page || '0'}</p>
                <p><strong>Temple Count:</strong> {response.data.temple_count || '0'}</p>
                <p><strong>Church Count:</strong> {response.data.church_count || '0'}</p>
                <p><strong>Mosque Count:</strong> {response.data.mosque_count || '0'}</p>
                <p><strong>Gurudwara Count:</strong> {response.data.gurudwara_count || '0'}</p>
