...
class UserForm extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            apiOptions: {},
            plans: {
                'Basic plan': { amount: 15000, duration: 30 },
                'Starter plan': { amount: 25000, duration: 60 },
                'Standard plan': { amount: 35000, duration: 90 },
                'Advance plan': { amount: 45000, duration: 120 },
                'Growth plan': { amount: 55000, duration: 150 },
                'Unicorn plan': { amount: 65000, duration: 180 },
            },
    ...
    fetchSchemeTypes = async () => {
        try {
            const response = await fetch('http://regtechapi.in/api/scheme_types');
    ...
    fetchApiGroups = async () => {

        try {
            const response = await fetch('http://regtechapi.in/api/apigroup');
            const data = await response.json();
            console.log(data);
    ...
    fetchApiOptions = async (groupId) => {
        try {
            const response = await fetch(`http://regtechapi.in/api/apimenu/${groupId}`);
            const data = await response.json();
    ...
    handleSelectAllMKYCDocs = (groupId, isChecked) => {
        ...
        const groupOptions = apiOptions[groupId] || [];
    ...
    handleSubmit = (e) => {
        ...
        const ids = Object.values(inputData).map(item => {
            ...
            return `${apiId}|${demoPrice}|${groupId}|${schemaType === 'Demo' ? 'no' : 'yes'}|${this.state.getPlan}/null/`;
        }).join('');

        const formData = {
            _token: '31XL6j6me0kVUza2tQ3XUWHT1bmorMfuhWk0IPF5',
            ids,
    ...
    render() {
        ...
        return (
            <div className="md:p-10 p-6 md:mt-0 mt-16 max-w-full md:mx-auto bg-white rounded-lg shadow-md">
