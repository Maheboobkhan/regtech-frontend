...
const PassportOCR = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${domain}/passport_ocr`, formData, { headers });
  ...
  function formatDate(dateString) {
    ...
    // Extract the year, month, and day from the YYMMDD format
    const year = dateString.substring(0, 2);  // First two digits (YY)
    const month = dateString.substring(2, 4); // Middle two digits (MM)
    const day = dateString.substring(4, 6);   // Last two digits (DD)
  
    // Determine the full year (assuming 19 for '75', and so on)
            ...
            <>
              {response.status_code === 200 && (
                <div className="bg-green-100 text-green-800 p-3 rounded mt-4">
                  ...
                  <p><strong>Date Of Birth:</strong> {formatDate(response.passport_verification.mrz_info.date_of_birth_yymmdd)}</p>
<p><strong>Expiration Date:</strong> {formatDate(response.passport_verification.mrz_info.expiration_date_yymmdd)}</p>
