...
class ChatComponent extends Component {
    constructor(props) {
        ...
        this.state = {
            ...
            pollingInterval: 10000, // Polling interval in milliseconds (2 seconds)
            isPolling: false // New state to control polling
        };
    }
    ...
    componentDidMount() {
        ...
        // Polling will now be started conditionally
    }
    ...
    componentWillUnmount() {
        this.stopPolling(); // Ensure polling is stopped on unmount
    }

    startPolling = () => {
        if (this.state.isPolling) return; // Prevent multiple intervals
    ...
    handleFormSubmit = async (e) => {
        ...
        try {
            ...
            this.startPolling(); // Start polling after form submission
        } catch (error) {
    ...
    handleClientMessage = async (e) => {
        ...
        try {
            ...
            this.startPolling(); // Start polling after sending a client message
        } catch (error) {
    ...
    handleOwnerMessage = async (e) => {
        ...
        try {
            ...
            this.startPolling(); // Start polling after sending an owner message
        } catch (error) {
            console.error("There was an error posting the message!", error);
        }
    };
    ...
    handleChatClick = (chatId) => {
        ...
        this.startPolling(); // Start polling when a chat is selected
    };
                                ...
                                <div className="md:w-80 w-[90vw] max-w-sm bg-white shadow-lg rounded-lg z-50">
                                    ...
                                    <div className="p-4 h-64 overflow-y-auto border-b border-gray-200">
                                        {this.state.messages?.messages?.map((msg, index) => (
                                            <div
                                                key={index}
                                                className={`mb-2 p-2 rounded-lg bg-gray-200 ${msg?.sender === 'owner' ? 'w-3/4 ml-auto': 'w-3/4 mr-auto'} text-black`}
                                            >
                            ...
                            <div className="flex flex-col h-full">
                                ...
                                <div className="p-4 h-64 overflow-y-auto border-b border-gray-200">
                                    {this.state.messages?.messages?.map((msg, index) => (
                                        <div
                                            key={index}
                                            className={`mb-2 p-2 rounded-lg bg-gray-200 ${msg?.sender === 'owner' ? 'w-3/4 mr-auto': 'w-3/4 ml-auto'} text-black`}
                                        >
                                            <div className="flex flex-col">
                                                <div className=''>
                                                    {msg?.message}
                                ...
                                {/* form */}
                                {isFormOpen && isUser === false && <div className="fixed bottom-20 right-4 bg-white shadow-lg rounded-lg p-4 z-1500">
                                    <h2 className="text-xl font-semibold text-black mb-4">Fill Form to talk to us</h2>
